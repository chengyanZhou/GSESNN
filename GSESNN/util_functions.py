...
class MyDynamicDataset(Dataset):
    ...
    def get(self, idx):
        ...
        return construct_pyg_graph(*tmp[0:7])
...
def subgraph_extraction_labeling(ind, Arow, Acol, label=1, h=1):
    ...
    return u, v, r, node_labels, max_node_label, label, node


def construct_pyg_graph(u, v, r, node_labels, max_node_label, y, node):
    ...
    #node = torch.IntTensor(node)
    data = Data(x, edge_index, edge_attr=edge_type, y=y, node=node)
...
def PyGGraph_to_nx(data):
    ...
    node_attributes = dict(zip(range(data.num_nodes), data.node.numpy()))
    nx.set_node_attributes(g, name='node_info', values=node_attributes)
...
def knn_graph(disMat, k):
    ...
    adj = ssp.coo_matrix((np.ones(edges.shape[0]), (edges[:, 0], edges[:, 1])),
...
def loadsims(data_name, k):
    root_path = os.path.dirname(os.path.abspath(__file__))
    if data_name in ['Gdataset', 'Cdataset']:
        path_dr = os.path.join(root_path, 'drug_data/{}'.format(data_name) + '/sims_drug.txt')
        path_dis = os.path.join(root_path, 'drug_data/{}'.format(data_name) + '/sims_disease.txt')
        sims_dr = np.loadtxt(path_dr)
        sims_dis = np.loadtxt(path_dis)
    elif data_name == 'lrssl':
        path_dr = os.path.join(root_path, 'drug_data/{}'.format(data_name) + '/sims_drug.txt')
        path_dis = os.path.join(root_path, 'drug_data/{}'.format(data_name) + '/sims_disease.txt')
        sims_dr = pd.read_table(path_dr, index_col=0).values
        sims_dis = pd.read_table(path_dis, index_col=0).values
    else:
        path_dr = os.path.join(root_path, 'drug_data/{}'.format(data_name) + '/sims_drug.csv')
        path_dis = os.path.join(root_path, 'drug_data/{}'.format(data_name) + '/sims_disease.csv')
        data_dr = pd.read_csv(path_dr, header=None)
        data_dis = pd.read_csv(path_dis, header=None)
        sims_dr = data_dr.values
        sims_dis = data_dis.values
    
    drug_adj = knn_graph(sims_dr, k)
    drug_graph = normalize(drug_adj + ssp.eye(drug_adj.shape[0]))

    disease_adj = knn_graph(sims_dis, k)
    disease_graph = normalize(disease_adj + ssp.eye(disease_adj.shape[0]))

    return sims_dr,sims_dis, drug_graph.toarray(), disease_graph.toarray()
